...
class RulesModel(BiddingModel):
    """Rule-based bidding model using learned/deduplicated criteria.
    
    This is the DEFAULT rules model. It uses merged rules from 
    bbo_bt_merged_rules.parquet (learned and deduplicated criteria for seat 1),
    combined with CSV overlay for customization.
    The actual matching is handled in the handlers.
    """
    ...
    @property
    def description(self) -> str:
        return "Base + Learned + Overlay: Full rules pipeline"
    
    @property
    def author(self) -> str:
        return "ML-Contract-Bridge"
    ...
    def get_auction(
        ...
        """Auction matching is handled by the arena handlers."""
        # The actual logic is in handle_bidding_arena (uses merged_rules_lookup)
        # This stub allows the model to be registered and validated
        return None
    
    def is_available(self, state: Dict[str, Any]) -> bool:
        """Check if bt_seat1_df (compiled BT with merged rules) is loaded."""
        # NOTE: merged_rules_lookup was removed; rules are now pre-compiled in bt_seat1_df
        return state.get("bt_seat1_df") is not None


class RulesLearnedModel(BiddingModel):
    """Rule-based bidding model using Base + Learned (no CSV overlay).
    
    This shows the effect of the machine learning pipeline on top of 
    the base GIB rules, without any manual CSV overrides.
    """
    ...
    @property
    def display_name(self) -> str:
        return "Rules (Base+Learned)"
    
    @property
    def description(self) -> str:
        return "Base + Learned: GIB rules with ML-discovered criteria (no CSV overlay)"
...
class RulesBaseModel(BiddingModel):
    """Rule-based bidding model using only Base rules.
    
    This is the raw GIB base criteria without any learned improvements
    or CSV overlays.
    """
    ...
    @property
    def display_name(self) -> str:
        return "Rules (Base Only)"
    
    @property
    def description(self) -> str:
        return "Base Only: Original GIB criteria (no learning, no overlay)"
...
MODEL_REGISTRY.register(RulesLearnedModel())
MODEL_REGISTRY.register(RulesBaseModel())
...
from bbo_bidding_models import BiddingModel, MODEL_REGISTRY
