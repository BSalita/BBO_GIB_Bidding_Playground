...
class RawRulesModel(BiddingModel):
    """Rule-based bidding model using raw GIB-style auction tables (no learned criteria).
    ...
    Uses original BT criteria without learned/merged modifications.
    """
    ...
    @property
    def display_name(self) -> str:
        return "Raw Rules (GIB Original)"
    
    @property
    def description(self) -> str:
        return "GIB-style rule-based bidding using original pre-computed auction tables (no learned criteria)"
...
class RulesModel(BiddingModel):
    ...
    @property
    def display_name(self) -> str:
        return "Rules (Learned + CSV)"
    
    @property
    def description(self) -> str:
        return "Rule-based bidding using learned criteria from bbo_bt_merged_rules.parquet + CSV overlay"
    ...
    def is_available(self, state: Dict[str, Any]) -> bool:
        """Check if merged_rules_lookup is loaded."""
        return state.get("merged_rules_lookup") is not None


# Register the built-in models
MODEL_REGISTRY.register(RulesModel())
MODEL_REGISTRY.register(RawRulesModel())
